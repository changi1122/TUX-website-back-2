...
public interface AuthControllerDocs {
    ...
    LoginResponse login(HttpServletResponse response, @RequestBody LoginRequest loginRequest);

    @Operation(method = "DELETE", summary = "로그아웃", description = "로그아웃하여 토큰이 저장된 쿠키를 삭제합니다.")
    void logout(HttpServletResponse response, @AuthenticationPrincipal User user);
